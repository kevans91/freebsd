...
#ifdef __FreeBSD__
	struct callout			 swofs_flow_callout;
#else
...
VNET_DECLARE(uma_zone_t, swfcl_zone);
VNET_DEFINE(uma_zone_t, swpld_zone);

void
swofp_attach(void)
{

	V_swpld_zone = uma_zcreate("swpld", sizeof(struct swofp_pipeline_desc),
...
swofp_create(struct switch_softc *sc)
{
...
#ifdef __FreeBSD__
	callout_init(&swofs->swofs_flow_callout, 1);
	callout_reset(&swofs->swofs_flow_callout, 10 * hz, swofp_timer, sc);
#else
	...
	bpfattach2(sc->sc_if, DLT_OPENFLOW, sizeof(struct ofp_header),
	    &sc->sc_ofbpf);
...
#ifdef __FreeBSD__
	callout_drain(&swofs->swofs_flow_callout);
#else
...
swofp_ioctl(struct ifnet *ifp, unsigned long cmd, caddr_t data)
{
	...
	case SIOCSWSDPID:
#ifdef __FreeBSD__
		if ((error = priv_check(td, PRIV_NET_SWITCH)) != 0)
#else
...
swofp_pipeline_desc_create(struct switch_flow_classify *swfcl)
{
	...
	swpld = uma_zalloc(V_swpld_zone, M_NOWAIT|M_ZERO);
	if (swpld == NULL)
		return (NULL);
    ...
    struct swofp_group_entry *swge)
{
	...
	free(swge->swge_buckets, M_DEVBUF);
	free(swge, M_DEVBUF);
...
#ifdef __FreeBSD__
	callout_schedule(&swofs->swofs_flow_callout, 10 * hz);
#else
	timeout_add_sec(&swofs->swofs_flow_timeout, 10);
#endif
}
...
swofp_expand_8021q_tag(struct mbuf *m)
{
	...
	return (ether_vlanencap(m, m->m_pkthdr.ether_vtag));
}
    ...
    struct swofp_pipeline_desc *swpld, struct ofp_action_header *oah)
{
	...
	if (m == NULL)
		return (NULL);
	m_copyback2(m, 0, sizeof(eh), (caddr_t)&eh, M_NOWAIT);
    ...
    struct swofp_pipeline_desc *swpld, struct ofp_action_header *oah)
{
	...
	M_PREPEND(m, sizeof(evh), M_NOWAIT);
	if (m == NULL)
		return (NULL);

	m_copyback2(m, 0, sizeof(evh), (caddr_t)&evh, M_NOWAIT);
    ...
    struct swofp_pipeline_desc *swpld, struct ofp_action_header *oah)
{
	...
	DPRINTF(sc, "Out prelim\n");
	if ((m = swofp_apply_set_field(m, swpld)) == NULL)
		return (NULL);

	oao = (struct ofp_action_output *)oah;
	DPRINTF(sc, "Out! ao_port == %u\n", ntohl(oao->ao_port));
    ...
    struct switch_flow_classify *pre_swfcl, struct switch_flow_classify *swfcl)
{
	...
	M_PREPEND(m, off, M_NOWAIT);
	if (m == NULL)
		return (NULL);

	m_copyback2(m, 0, off, eh_bk, M_NOWAIT);
    ...
    struct swofp_pipeline_desc *swpld, struct ofp_action_header *oah)
{
	...
	if (oxm->oxm_class == htons(OFP_OXM_C_NXM_1)) {
		...
		case OFP_XM_NXMT_TUNNEL_IPV6_DST:
			if (pre_swfcl->swfcl_tunnel)
				break;
			pre_swfcl->swfcl_tunnel = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_ETH_TYPE:
			if (pre_swfcl->swfcl_ether)
				break;
			pre_swfcl->swfcl_ether = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_VLAN_PCP:
			if (pre_swfcl->swfcl_vlan)
				break;
			pre_swfcl->swfcl_vlan = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_ARP_OP:
			if (pre_swfcl->swfcl_arp)
				break;
			pre_swfcl->swfcl_arp = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
			if (swfcl->swfcl_arp == NULL)
				goto failed;
		...
		case OFP_XM_T_IP_PROTO:
			if (swfcl->swfcl_ipv4) {
				if (pre_swfcl->swfcl_ipv4)
					break;
				pre_swfcl->swfcl_ipv4 = uma_zalloc(V_swfcl_zone,
				    M_NOWAIT|M_ZERO);
				...
				if (pre_swfcl->swfcl_ipv6)
					break;
				pre_swfcl->swfcl_ipv6 = uma_zalloc(V_swfcl_zone,
				    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_UDP_DST:
			if (pre_swfcl->swfcl_udp)
				break;
			pre_swfcl->swfcl_udp = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_TCP_DST:
			if (pre_swfcl->swfcl_tcp)
				break;
			pre_swfcl->swfcl_tcp = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_ICMPV4_TYPE:
			if (pre_swfcl->swfcl_icmpv4)
				break;
			pre_swfcl->swfcl_icmpv4 = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_ICMPV6_TYPE:
			if (pre_swfcl->swfcl_icmpv6)
				break;
			pre_swfcl->swfcl_icmpv6 = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
		...
		case OFP_XM_T_IPV6_ND_TARGET:
			if (pre_swfcl->swfcl_nd6)
				break;
			pre_swfcl->swfcl_nd6 = uma_zalloc(V_swfcl_zone,
			    M_NOWAIT|M_ZERO);
			if (pre_swfcl->swfcl_nd6 == NULL)
				goto failed;
    ...
    struct swofp_pipeline_desc *swpld, struct ofp_action_header *oah)
{
	...
	DPRINTF(sc, "Executing handler for %d (%s)\n", ntohs(oah->ah_type),
	    handler->action == &swofp_action_output ? "out" : "/nope?");
    ...
    struct mbuf *m)
{
	...
	DPRINTF(sc, "Executing action\n");
	m = swofp_execute_action_set(sc, m, swpld);
 out:
	DPRINTF(sc, "Swift exit!\n");
	m_freem(m);
...
swofp_input(struct switch_softc *sc, struct mbuf *m)
{
	...
	if (sc->sc_ofbpf)
		switch_mtap(sc->sc_ofbpf, m, BPF_D_IN,
...
swofp_output(struct switch_softc *sc, struct mbuf *m)
{
	...
	if (sc->sc_ofbpf)
		switch_mtap(sc->sc_ofbpf, m, BPF_D_OUT,
		    swofs->swofs_datapath_id);
    ...
    uint16_t type, uint16_t code)
{
	...
	KASSERT(off == 0, ("%s: off must also be 0", __func__));
...
swofp_recv_features_req(struct switch_softc *sc, struct mbuf *m)
{
	...
	m_copyback2(m, 0, sizeof(swf), (caddr_t)&swf, M_WAITOK);
...
swofp_recv_config_req(struct switch_softc *sc, struct mbuf *m)
{
	...
	if (m_copyback2(m, 0, sizeof(osc), (caddr_t)&osc, M_NOWAIT)) {
		m_freem(m);
		return (-1);
	}
...
swofp_mpmsg_reply_create(struct ofp_multipart *req, struct swofp_mpmsg *swmp)
{
	...
	MGETHDR(body, M_NOWAIT, MT_DATA);
	if (body == NULL)
		goto error;
	MCLGET(body, M_NOWAIT);
...
swofp_mpmsg_put(struct swofp_mpmsg *swmp, caddr_t data, size_t len)
{
	...
	KASSERT(swmp->swmp_hdr != NULL, ("%s: header prepopulated", __func__));

	m = MBUF_LIST_LAST(&swmp->swmp_body);
	if (m == NULL)
		return (ENOBUFS);
	...
	if ((error = m_copyback2(m, m->m_pkthdr.len, len, data, M_NOWAIT)))
		return (error);
    ...
    struct swofp_flow_entry *swfe)
{
	 ...
	 * with ofp_matches
	 */
	if ((error = m_copyback2(m, off, (sizeof(ofs) -
	    sizeof(struct ofp_match)), (caddr_t)&ofs, M_NOWAIT)))
		goto failed;
	...
	if ((error = m_copyback2(m, off, omlen, (caddr_t)swfe->swfe_match,
	    M_NOWAIT)))
		goto failed;
	off += omlen;
	if ((error = m_copyback2(m, off, pad, pad_data, M_NOWAIT)))
		goto failed;
	off += pad;
	 ...
	 */
	if (swfe->swfe_goto_table) {
		if ((error = m_copyback2(m, off,
		    ntohs(swfe->swfe_goto_table->igt_len),
		    (caddr_t)swfe->swfe_goto_table, M_NOWAIT)))
			goto failed;
	...
	if (swfe->swfe_write_metadata) {
		if ((error = m_copyback2(m, off,
		    ...
		    (caddr_t)swfe->swfe_write_metadata, M_NOWAIT)))
			goto failed;
	...
	if (swfe->swfe_apply_actions) {
		if ((error = m_copyback2(m, off,
		    ...
		    (caddr_t)swfe->swfe_apply_actions, M_NOWAIT)))
			goto failed;
	...
	if (swfe->swfe_write_actions) {
		if ((error = m_copyback2(m, off,
		    ...
		    (caddr_t)swfe->swfe_write_actions, M_NOWAIT)))
			goto failed;
	...
	if (swfe->swfe_clear_actions) {
		if ((error = m_copyback2(m, off,
		    ...
		    (caddr_t)swfe->swfe_clear_actions, M_NOWAIT)))
			goto failed;
...
swofp_mp_recv_port_stats(struct switch_softc *sc, struct mbuf *m)
{
	...
	TAILQ_FOREACH(swpo, &sc->sc_swpo_list, swpo_list_next) {
		...
		postat.pt_rx_packets = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_IPACKETS));
		postat.pt_tx_packets = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_OPACKETS));
		postat.pt_rx_bytes = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_OBYTES));
		postat.pt_tx_bytes = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_IBYTES));
		postat.pt_rx_dropped = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_IQDROPS));
		postat.pt_tx_dropped = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_OQDROPS));
		postat.pt_rx_errors = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_IERRORS));
		postat.pt_tx_errors = htobe64(if_get_counter_default(ifs,
		    IFCOUNTER_OERRORS));
		postat.pt_rx_frame_err = htobe64(0);
		postat.pt_rx_over_err = htobe64(0);
		postat.pt_rx_crc_err = htobe64(0);
		postat.pt_collision = htobe64(if_get_counter_default(ifs,
...
swofp_table_features_put_oxm(struct mbuf *m, int *off, uint16_t tp_type)
{
	...
	if (m_copyback2(m, *off, sizeof(tp), (caddr_t)&tp, M_NOWAIT))
	...
	*off += sizeof(tp);
	...
	for (i = 0 ; i < nitems(ofp_oxm_handlers); i++) {
		...
		if (m_copyback2(m, *off, sizeof(oxm), (caddr_t)&oxm, M_NOWAIT))
			return (OFP_ERRREQ_MULTIPART_OVERFLOW);
		*off += sizeof(oxm);
	}
	 ...
	 */
	if ((supported & 0x1) == 0) {
		if (m_copyback2(m, *off, sizeof(padding),
...
swofp_table_features_put_actions(struct mbuf *m, int *off, uint16_t tp_type)
{
	...
	*off += sizeof(tp);
	...
	for (i = 0 ; i < nitems(ofp_action_handlers); i++) {
		...
		if (m_copyback2(m, *off, actionlen,
	...
	if (padsize) {
		...
		if (m_copyback2(m, *off, padsize, (caddr_t)&padding, M_NOWAIT))
			return (OFP_ERRREQ_MULTIPART_OVERFLOW);
...
swofp_table_features_put_instruction(struct mbuf *m, int *off, uint16_t tp_type)
{
	...
	*off += sizeof(tp);
	...
	if (m_copyback2(m, *off, sizeof(instructions), (caddr_t)instructions,
	    M_NOWAIT))
		return (OFP_ERRREQ_MULTIPART_OVERFLOW);
