...
struct switch_dev {
	...
	struct mbufq		 swdev_outq;
	struct mtx		 swdev_mtx;
...
struct switch_softc {
	...
	struct bpf_if			*sc_ofbpf;		/* DLT_OPENFLOW */
...
struct mbuf
	*switch_flow_classifier(struct mbuf *, uint32_t,
	    struct switch_flow_classify *);
void	 switch_mtap(struct bpf_if *, struct mbuf *, int, uint64_t);
...
extern const char if_switchname[];
extern struct cdevsw switch_cdevsw;

#define	V_swfcl_zone	VNET(swfcl_zone)
#define	V_swpld_zone	VNET(swpld_zone)
