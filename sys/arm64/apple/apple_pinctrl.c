...
#if 0
#define APPLE_PINCTRL_DEFAULT_IRQ_CAPS	\
	(GPIO_INTR_LEVEL_HIGH | GPIO_INTR_LEVEL_LOW | \
	GPIO_INTR_LEVEL_RISING | GPIO_INTR_EDGE_FALLING | GPIO_INTR_EDGE_BOTH)
#endif
#define	APPLE_PINCTRL_DEFAULT_IRQ_CAPS	0	/* NOT YET */

#define	APPLE_PINCTRL_DEFAULT_CAPS	\
	(GPIO_PIN_INPUT | GPIO_PIN_OUTPUT | APPLE_PINCTRL_DEFAULT_IRQ_CAPS)
...
static void	apple_pinctrl_intr(void *);
#if 0
void	*apple_pinctrl_intr_establish(void *, int *, int, struct cpu_info *,
	    int (*)(void *), void *, char *);
void	apple_pinctrl_intr_disestablish(void *);
void	apple_pinctrl_intr_enable(void *);
void	apple_pinctrl_intr_disable(void *);
void	apple_pinctrl_intr_barrier(void *);
#endif
...
apple_pinctrl_attach(device_t dev)
{
	...
	if (bus_setup_intr(dev, sc->sc_res[APPLE_PINCTRL_IRQRES],
	    INTR_TYPE_CLK | INTR_MPSAFE, NULL, apple_pinctrl_intr, sc,
	    &sc->sc_intrhand)) {
...
apple_pinctrl_intr(void *arg)
{
	...
	struct apple_pinctrl_irqsrc *ih;
...
{
	struct apple_pinctrl_irqsrc *ih = cookie;
...
void
apple_pinctrl_intr_barrier(void *cookie)
{
	struct apple_pinctrl_irqsrc *ih = cookie;
	struct apple_pinctrl_softc *sc = ih->ih_sc;
...
#if 0
	/* PIC interface */
	DEVMETHOD(pic_disable_intr,	apple_pinctrl_pic_disable_intr),
	DEVMETHOD(pic_enable_intr,	apple_pinctrl_pic_enable_intr),
	DEVMETHOD(pic_map_intr,		apple_pinctrl_pic_map_intr),
	DEVMETHOD(pic_setup_intr,	apple_pinctrl_pic_setup_intr),
	DEVMETHOD(pic_teardown_intr,	apple_pinctrl_pic_teardown_intr),
	DEVMETHOD(pic_post_filter,	apple_pinctrl_pic_post_filter),
	DEVMETHOD(pic_post_ithread,	apple_pinctrl_pic_post_ithread),
	DEVMETHOD(pic_pre_ithread,	apple_pinctrl_pic_pre_ithread),
#endif
