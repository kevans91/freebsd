    ...
    uint8_t type)
{
	...
	case OFP_XM_T_TUNNEL_ID:
		...
		if (hasmask)
			log_debug("\t\t%"PRIu64" mask %#16"PRIx64,
			    be64toh(*ui64), be64toh(*(ui64 + 1)));
		else
			log_debug("\t\t%"PRIu64, be64toh(*ui64));
		break;
    ...
    struct ofp_header *oh, struct ibuf *ibuf)
{
	...
	if ((pin = ibuf_seek(ibuf, off, sizeof(*pin))) == NULL)
		return (-1);
	log_debug("\tbuffer %s length %u reason %s table %s cookie 0x%#016"PRIx64,
    ...
    struct ofp_header *oh, struct ibuf *ibuf)
{
	...
	log_debug("\tdatapath_id %#016"PRIx64" nbuffers %u ntables %d aux_id %d "
	    "capabilities %#08x",
	...
	case OFP_INSTRUCTION_T_WRITE_META:
		...
		*off += len;
		log_debug("\tinstruction %s length %lu "
		    "metadata %#016"PRIx64" mask %#016"PRIx64,
		    print_map(type, ofp_instruction_t_map), len,
    ...
    struct ofp_header *oh, struct ibuf *ibuf)
{
	...
	log_debug("\tcommand %s table %s timeout (idle %d hard %d) "
	    ...
	    "flags %#04x cookie %#016"PRIx64" mask %#016"PRIx64,
    ...
    struct ofp_header *ih, struct ibuf *ibuf)
{
	...
	log_debug("cookie %#016"PRIx64" priority %d reason %s table_id %s "
	    "duration(%u sec, %u nsec) timeout idle %d hard %d "
	    "packet %"PRIu64" byte %"PRIu64,
...
ofp13_validate_tableproperty(struct ibuf *ibuf, off_t off, int remaining)
{
 ...
 next_table:
	...
	log_debug("\t\ttable features length %d tableid %s "
	    " name \"%s\" metadata match %#016"PRIx64" write %#016"PRIx64
    ...
    struct ofp_header *oh, struct ibuf *ibuf)
{
 ...
 read_next_flow:
		...
		log_debug("\tflow length %d table_id %s duration sec %u "
		    ...
		    "cookie %#016"PRIx64" packet count %"PRIu64" byte count %"PRIu64,
    ...
    struct ofp_header *oh, struct ibuf *ibuf)
{
	...
	case OFP_MP_T_FLOW:
		...
		log_debug("\ttable_id %s out_port %s out_group %s "
		    "cookie %#016"PRIx64" mask %#016"PRIx64" match type %s length %d "
		    "(padded to %d)",
